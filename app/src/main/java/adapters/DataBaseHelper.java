...
public class DataBaseHelper extends SQLiteOpenHelper
{
    ...
    public void addNewAddress(String address_name, String formated_address, double address_lat, double address_lng)
    {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues contentValues = new ContentValues();
        contentValues.put(ADDRESS_NAME, address_name);
        contentValues.put(ADDRESS_FORMATED, formated_address);
        contentValues.put(ADDRESS_LAT, address_lat);
        contentValues.put(ADDRESS_LNG, address_lng);
        db.insert(TABLE_NAME_ADDRESS,null,contentValues);
        db.close();
    }

    public ArrayList<DbSavedAddress> getAllSavedAddress()
    {
        ArrayList<DbSavedAddress> dbSavedAdrs = new ArrayList<>();
        String addressQuesry = "SELECT DISTINCT "+ ADDRESS_NAME+", "+ADDRESS_FORMATED+", "+ADDRESS_LAT+", "+ADDRESS_LNG+" FROM " +TABLE_NAME_ADDRESS;
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor cursor = db.rawQuery(addressQuesry,null);

        if(cursor.moveToFirst())
        {
            do
            {
                DbSavedAddress dbSavedAdres = new DbSavedAddress();
                dbSavedAdres.setAddress_name(cursor.getString(0));
                dbSavedAdres.setAddress_formate(cursor.getString(1));
                dbSavedAdres.setAddress_lat(cursor.getDouble(2));
                dbSavedAdres.setAddress_lng(cursor.getDouble(3));
                dbSavedAdrs.add(dbSavedAdres);
            }while (cursor.moveToNext());
        }
        db.close();
        cursor.close();
